...
| **MobilityNebula** | 6 systest-confirmed | 147 wired · compile-verified | 7.6 % wired |
...
Beyond function coverage, the three platforms run the **BerlinMOD** reference queries in
three streaming forms — continuous, windowed, and snapshot — with the **snapshot form
anchored to the batch (SQL-engine) result at the same scale factor**. Snapshot results
agree with batch, which links streaming parity to [database parity](database-parity.md).
...
Benchmark results are published here as each clean run completes; the streaming benchmark
harnesses live with the parity tooling in the
[MobilityFlink](https://github.com/MobilityDB/MobilityFlink),
[MobilityKafka](https://github.com/MobilityDB/MobilityKafka), and
[MobilityNebula](https://github.com/MobilityDB/MobilityNebula) repositories. SQL-engine
timings appear in the companion [databases — parity & benchmarks](database-parity.md) page.
...
*Measured against the `accumulate/parity-1.4` MEOS build. Status as of 2026-05-23;
NebulaStream coverage advances as further operator families are wired.*
